...
const App: React.FC = () => {
  ...
  const handleNewsClick = async (newsItem: NewsItem) => {
    ...
    try {
      // Получаем полную новость с content_html
      console.log('🔍 Начинаем загрузку полной новости для ID:', newsItem.id);
      const fullNews = await fetchNewsById(newsItem.id);
      console.log('🔍 Получена полная новость:', fullNews);
      console.log('🔍 content_html в полной новости:', fullNews.content_html ? 'Есть' : 'Нет');
      console.log('🔍 content_html длина:', fullNews.content_html?.length || 0);
      console.log('Полученные данные из API:', {
        background_image: fullNews.background_image,
        background_color: fullNews.background_color,
        icon: fullNews.icon,
        gradient_start: fullNews.gradient_start,
        gradient_end: fullNews.gradient_end,
        media_count: fullNews.media?.length || 0,
        content_html: fullNews.content_html ? 'Есть HTML контент' : 'Нет HTML контента',
        content_length: fullNews.content?.length || 0
      });
      
      setSelectedNews(fullNews);
    } catch (error) {
      console.error('❌ Ошибка при получении полной новости:', error);
      // Если не удалось получить полную новость, используем базовую
      console.log('⚠️ Используем базовую новость как fallback');
